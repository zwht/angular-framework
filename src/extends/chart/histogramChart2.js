...
function BrokenLineChart(config) {
    ...
    for (i = 0; i < length; i++) {
        ...
        if (data[i].data[0] === 1) data[i].data[0] = 0;
        if (data[i].data[1] === 1) data[i].data[1] = 0;
        ls[0] = scaleX.getRange(i + 0.5);
        ls[1] = scaleY.getRange(data[i].data[0]);
        ls[2] = scaleX.getRange(i + 0.5);
        ls[3] = scaleY.getRange(data[i].data[1]);
    ...
    for (i = 0; i < length; i++) {
        for (j = 0; j < 2; j++) {
            ...
            var xStart = polyLineArr[i][0] - oneWidth;
            var yStart = height;
            var xEnd = polyLineArr[i][0] - oneWidth;
            ...
            if (j === 1) {
                texCol = "#f7b537";
                xStart = polyLineArr[i][2] + oneWidth;
                yStart = height;
                xEnd = polyLineArr[i][2] + oneWidth;
                yEnd = polyLineArr[i][3];
            }

            var Test = data[i].data[j];
            if (Test === 0) Test = 1;
